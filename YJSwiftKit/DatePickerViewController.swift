...
class DatePickerViewController: UIViewController {
    ...
    private func drawMyView() {
        ...
        picker = UIPickerView(frame: CGRect(x: 0, y: 30, width: ScreenInfo.Width, height: 210))
...
extension DatePickerViewController:UIViewControllerTransitioningDelegate {
    func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = DatePickerPresentAnimated(type: .present)
        return animated
    }
    func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = DatePickerPresentAnimated(type: .dismiss)
        return animated
    }
}
...
class DatePickerPresentAnimated: NSObject,UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        switch type {
        case .present:
            guard let toVC = transitionContext.viewController(forKey: .to) as? DatePickerViewController else {
                return
            }
//            let toVC : DatePickerViewController = transitionContext.viewController(forKey: .to) as? DatePickerViewController
        ...
        case .dismiss:
            guard let toVC = transitionContext.viewController(forKey: .from) as? DatePickerViewController else {
                return
            }
            UIView.animate(withDuration: 0.25, animations: {
