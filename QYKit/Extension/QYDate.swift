...
extension Date {
    ///date --> string
    public func yi_toString(_ dateFormat : String = "yyyy-MM-dd HH:mm:ss",_ timeZone : TimeZone = NSTimeZone.system) -> String {
...
extension Date {
    ///加几天
    public func yi_addingDay(_ days: Int) -> Date? {
    ...
    ///减几天
    public func yi_subtractingDay(_ days: Int) -> Date? {
        var c = DateComponents()
        c.day = days * -1
        let calender = Calendar(identifier: .chinese)
    ...
    ///增加几小时
    public func yi_addingHours(_ dHours: Int) -> Date? {
    ...
    ///减少几小时
    public func yi_subtractingHours(_ dHours: Int) -> Date? {
        let aTimeInterval = TimeInterval(timeIntervalSinceReferenceDate - Double(60 * 60 * dHours))
        let newDate = Date(timeIntervalSinceReferenceDate: aTimeInterval)
        return newDate
    }
